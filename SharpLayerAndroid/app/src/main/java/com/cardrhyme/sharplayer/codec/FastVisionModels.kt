...
/**
 * Faster Option D perception runtime.
 *
 * Tries the TensorFlow Lite GPU delegate independently for each model and
 * falls back to the normal CPU/XNNPACK path when the device or model rejects it.
 */
class FastVisionModels(context: Context) : Closeable {
    ...
    private val depth = createRuntime("midas.tflite", "MiDaS")
    private val segmentation = createRuntime("deeplab_v3.tflite", "DeepLab")

    val acceleratorSummary: String
        get() = "${depth.label} + ${segmentation.label}"

    fun estimateDepth(bitmap: Bitmap, outputWidth: Int, outputHeight: Int): FloatArray {
        val interpreter = depth.interpreter
    ...
    fun segment(bitmap: Bitmap, outputWidth: Int, outputHeight: Int): ByteArray {
        val interpreter = segmentation.interpreter
        val inputTensor = interpreter.getInputTensor(0)
        val shape = inputTensor.shape()
        ...
        val spatialHeight = if (outputShape.size >= 3) outputShape[outputShape.size - 3] else modelHeight
        val spatialWidth = if (outputShape.size >= 2) outputShape[outputShape.size - 2] else modelWidth
        val channels = if (outputShape.size >= 4) outputShape.last() else 1
    ...
    override fun close() {
        depth.interpreter.close()
        segmentation.interpreter.close()
        depth.delegate?.close()
