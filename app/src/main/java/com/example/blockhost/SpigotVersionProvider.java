...
public final class SpigotVersionProvider {
    ...
    public synchronized JSONArray getVersions(boolean forceRefresh) throws Exception {
        if (!forceRefresh && cacheFile.isFile() && System.currentTimeMillis() - cacheFile.lastModified() < 21600000L) {
            try { return new JSONArray(Files.readString(cacheFile.toPath(), StandardCharsets.UTF_8)); } catch (Exception ignored) {}
        }
        try {
            JSONArray versions = fetchOfficialVersions();
            Files.writeString(cacheFile.toPath(), versions.toString(), StandardCharsets.UTF_8);
            return versions;
        } catch (Exception networkError) {
            if (cacheFile.isFile()) try { return new JSONArray(Files.readString(cacheFile.toPath(), StandardCharsets.UTF_8)); } catch (Exception ignored) {}
    ...
    private JSONArray fetchOfficialVersions() throws Exception {
        ...
        String html = new String(connection.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
    ...
    private static int compareVersions(String a,String b) {
        ...
        for(int i=0;i<Math.max(av.length,bv.length);i++) { int ai=i<av.length?av[i]:0,bi=i<bv.length?bv[i]:0; if(ai!=bi)return Integer.compare(ai,bi); }
        return 0;
    }
