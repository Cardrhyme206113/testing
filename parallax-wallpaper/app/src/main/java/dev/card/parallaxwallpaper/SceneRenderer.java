...
public final class SceneRenderer {
    ...
    public void drawFrame() {
        ...
        if(!ready) return;
        GLES30.glDisable(GLES30.GL_DEPTH_TEST); GLES30.glUseProgram(bgProgram); GLES30.glActiveTexture(GLES30.GL_TEXTURE0); GLES30.glBindTexture(GLES30.GL_TEXTURE_2D,textures[centerTextureLayer]); GLES30.glUniform1i(bgTex,0); GLES30.glUniform1f(bgScreenAspect,(float)width/height); GLES30.glUniform1f(bgTexAspect,textureAspect); GLES30.glDrawArrays(GLES30.GL_TRIANGLES,0,3); GLES30.glEnable(GLES30.GL_DEPTH_TEST);
    ...
    private void loadCurrent() throws Exception {
        ...
        JSONArray tx=p.getJSONArray("textures"); textures=new int[tx.length()]; GLES30.glGenTextures(textures.length,textures,0);
        for(int i=0;i<textures.length;i++){
            Bitmap b=BitmapFactory.decodeFile(new File(dir,tx.getString(i)).getAbsolutePath()); if(b==null)throw new IOException("Could not decode texture "+i);
            ...
            GLUtils.texImage2D(GLES30.GL_TEXTURE_2D,0,b,0); GLES30.glGenerateMipmap(GLES30.GL_TEXTURE_2D); b.recycle();
        }
        ready=true;
    }
